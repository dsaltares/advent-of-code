...
// The solution is taken from https://github.com/bhosale-ajay/adventofcode/blob/master/2021/ts/D23.ts.
...
const ascendingByEnergy = (a: State, b: State) => a.energy - b.energy;
...
const moveFromRoomToHallway = (
  ...
  return {
    ...
    energy: energy + energyNeededToMoveFromRoom + energyNeededToMoveInHallway,
  };
};
...
const getNextStates = ({ arrangement, energy }: State, rooms: Room[]) => {
  ...
  outer: for (let i = 0; i <= HALLWAY_SIZE; i++) {
    ...
    const energyNeededToMoveInHallway = Math.abs(i - co) * energyNeed[occupant];
    next.push({
      arrangement: updated.join(''),
      energy: energy + energyNeededToMoveInRoom + energyNeededToMoveInHallway,
    });
  }
  ...
  for (let ri = 0; ri < rooms.length; ri++) {
    ...
    for (let p = co - 1; p >= 0; p--) {
      ...
      next.push(moveFromRoomToHallway({ arrangement, energy }, room, co, p));
    }
...
const day23 = () => solve('DDCABACB', 50000);

export const day23PartTwo = () => solve('DDDDCCBABBAACACB', 50000);
