import { createInstructions, applyInstructions, countCubes } from './day22';
...
describe('applyInstructions', () => {
  it('applies single instruction and gets correct number of on cubes', () => {
    const data = 'on x=10..12,y=10..12,z=10..12';
    const instructions = createInstructions(data);
    const state = applyInstructions(instructions);
    const count = countCubes(state);
  ...
  it('applies all instructions on large sample input and gets correct number of on cubes', () => {
    const data = `
    ...
    expect(count).toEqual(2758514936282235);
  });
});
